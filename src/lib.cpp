...
void cameraSettingView(CameraView & cameraView, int x, int y, int radius, vector<vector<bool>> matrix) {
  int wasCornerTherei = -1;
  int wasCornerTherej = -1;

  for(int i = 0; i <= radius; i++) {
    if(x+i >= cameraView.height)  break;
    if(pointsDistance(x,y,x+i,y) > radius)  break;
    for(int j = 0; j <= radius; j++) {
      if(y+j >= cameraView.width)  break;
...
bool isCornerOnLine(int x1, int y1, int x2, int y2, vector<vector<bool>> matrix) {
     //auxiliary variables
